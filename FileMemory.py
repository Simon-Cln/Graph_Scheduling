...
class FileMemory:
    ...
    def file_reading(self, file_path=None):
        if file_path:
            try:
                ...
                print("Vous n'avez pas les droits d'accès pour lire le fichier {}.".format(file_path))
                return None
            except ValueError:
                print("Le format du fichier {} est incorrect.".format(file_path))
                return None

        else:
            file_txt = glob.glob("files/*.txt")
            ...
            nom_fichier = "files/file{}.txt".format(num_fichier)
            try:
